<template>
    <div class="pedal-container">
        <div class="box"></div>
        <div class="pedals">
            <div class="pedal" :class="item" v-for="(item,index) in pedals" :key="index" :style="{color:store.config.colors[item].color}">
                <n-icon :style="{filter: `drop-shadow(${'0 0 4px'+store.config.colors[item].color})`}">
